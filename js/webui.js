	...
	loadLabels: function(d) {
		...
		for(var i=0; i<keys.length; i++) {
			...
			if(!$$("-_-_-" + lbl + "-_-_-")){
				p.append($("<LI>")
					.attr("id", "-_-_-" + lbl + "-_-_-")
					.html('<div class="label-text inlabel">' + escapeHTML(lbl) + "&nbsp;(<span id=\"-_-_-" + lbl + "-_-_-c\">" + d[lbl] + "</span>)</div>")
					.mouseclick(theWebUI.labelContextMenu)
					.addClass("cat"));
			}
		}
...
//
	...
	showPanel: function(pnl,enable){
		...
		pnl.style.backgroundImage = "url("+this.getTable("trt").paletteURL+(enable ? "/assets/icons/hardware/svg/ic_keyboard_arrow_down_24px.svg)" : "/assets/icons/hardware/svg/ic_keyboard_arrow_right_24px.svg)");
	},
