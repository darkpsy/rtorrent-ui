...
plugin.updateLabalsImages = function(){
	$('#plabel_cont ul li').each( function(){
		...
		if(!$$("lbl_"+lbl))
			$(this).prepend( $('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="tfavicon label"><path d="M21.41 11.58l-9-9c-.36-.36-.86-.58-1.41-.58h-7c-1.1 0-2 .9-2 2v7c0 .55.22 1.05.59 1.42l9 9c.36.36.86.58 1.41.58.55 0 1.05-.22 1.41-.59l7-7c.37-.36.59-.86.59-1.41 0-.55-.23-1.06-.59-1.42zm-15.91-4.58c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>').attr("id", "lbl_"+lbl));
	});
}
	      ...
	      li.children("span").text(trackersLabels[lbl]);
			...
			else
			{
			  li = $('<li>').attr("id",'i'+lbl).
				...
				html('<div class="label-text inlabel">' + escapeHTML(lbl)+'&nbsp;(<span id="-'+lbl+'_c">'+trackersLabels[lbl]+'</span>)</div>').
